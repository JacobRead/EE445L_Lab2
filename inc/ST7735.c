...
// EE445L lab assignment
	...
	ST7735_FillScreen(0x0000); 										//make background black
	ST7735_SetCursor(0,0); 												//RESET the cursor
	printf("\n");
	ST7735_OutString(title); 											//print the title
	for(int i = 0; i<128; i++){
		for(int j=32; j<160; j++){
			ST7735_DrawPixel(i,j,bColor);							//Fills in the Background
		}
	}
	smallX = minX;																//stores min/max values in global variable
	smallY = minY;
	BigX = maxX;
	BigY = maxY;
}
...
// EE445L lab assignment
	...
	int Xscale = (128000/(BigX-smallX));					//scales pixels to min/max values
	int Yscale = (128000/(BigY-smallY));
//	printf("\n%d\n%d", Xscale,Yscale);
	
	int32_t X0 = smallX*(-1)*Xscale*(0.001);			//initialize coordinates at their respective (0,0)
	int32_t Y0 = (BigY*Yscale*(0.001))+32;
	
	int32_t X;
	int32_t Y;
	for(int i=0; i<num; i++){
		X = (Xscale*bufX[i]*0.001)+ X0;
		Y = Y0-(Yscale*bufY[i]*0.001);
		ST7735_DrawPixel(X,Y,color);								//Draws 4 pixels instead of 1 to appear smoother
